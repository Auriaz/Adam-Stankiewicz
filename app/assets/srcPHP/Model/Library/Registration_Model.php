...
class Registration_Model extends Model {
	...
	function Insert() {
		if (isset ($_POST['email'])){
			...
			//Hashowanie hasla
			$hash_key = Hash::create('sha256', $pass_1, HASH_KEY);

			$pass_hash = password_hash($hash_key, PASSWORD_DEFAULT 
			);
		   ...
		   $result = $this->Connect->query("SELECT id_user FROM uzytkownicy WHERE email='$email'");
		  ...
		  $result = $this->Connect->query("SELECT id_user FROM uzytkownicy WHERE user='$nick'");
		   ...
		   if($how_many_nicks > 0) {
			...
			if($all_OK == true) {
				...
				if($this->Connect->query("INSERT INTO uzytkownicy VALUES (NULL, '$nick', '$pass_hash', '$email', '$role' )")) {
